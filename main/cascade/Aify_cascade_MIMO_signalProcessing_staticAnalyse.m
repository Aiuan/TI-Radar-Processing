...
% 相机数据路径
image_path = 'K:\ourDataset\20210626match_radar_camera\image\match7.jpg';
...
%描述待处理数据文件目录
testList = strcat(input_path,'testList_staticAnalyse.txt');
%打开testList.txt文件
        ...
        for frameIdx = 1:1:min(numValidFrames,1+numFrames_toRun)
            ...
            if ~isempty(detection_results)%如果检测到点
                ...
                if length(angleEst) > 0%如果检测到角度结果
                    ...
                    subplot(2,3,1);
                    imshow(image_path);
                    temp = strsplit(image_path, filesep);
                    title(temp{end});
                    hold on;
            ...
            if SAVE_ON
                temp = strsplit(image_path, filesep);
                output_file_name = temp{end}(1:strfind(temp{end}, '.')-1);
                saveas(gcf, ['output/', output_file_name, '.png']);
            end
