...
%                         if cnt==1%如果是处理并存储的第一帧的结果
%                             scatter3(xyz(moveID,1),xyz(moveID,2),xyz(moveID,3),45,(xyz(moveID,4)),'filled');%x,y,z,doppler_corr
%                         else
%                             yz = [xyz_all{cnt}; xyz_all{cnt-1}];%yz似乎没有用到
...
%                         end
                        ...
                        %xlim([-20 20])
                        %ylim([1 maxRangeShow])%定义坐标系为y是雷达板正前方
                        %zlim([-4 4])
                        %zlim([-5 5])
                        xlabel('X (m)');
                        ylabel('Y (m)');
